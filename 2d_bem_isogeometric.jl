...
# ╔═╡ 3763e7cc-59af-11ec-1806-e9a28e4be8fe
using GaussQuadrature, LinearAlgebra, SpecialFunctions, ForwardDiff, Dierckx, Plots

# ╔═╡ 625be4dd-86b1-4be3-a923-52acac07b0b1
md"
# Interactive 2D BEM
"

# ╔═╡ 780a2c52-97c4-470c-8889-f0d0994148f7
md"
###### Defining functions
"

# ╔═╡ ae5fbc6c-d464-4083-9133-1c44deec0c12
begin
#=====================================================================================
                                Helper Functions                          
=====================================================================================#
...
obj(p1,p2,k,d) = 1.0 - abs2((exp(-im*k*d) - p2/p1)/(p2/p1 - exp(im*k*d)))
#=====================================================================================
                                Basis Functions
=====================================================================================#
...
discLinear(ξ,β)    = [0.5 .- ξ/(2.0*β); 0.5 .+ ξ/(2.0*β)]
discQuadratic(ξ,β) = [ξ.*(ξ.-β)/(2.0*β^2);(β.-ξ).*(β.+ξ)/(β^2);ξ.*(ξ.+β)/(2.0*β^2)]
basisFunctionDerivative(func,ξ) = hcat(ForwardDiff.derivative.(func,ξ)...) # Derivs
...
function createPhysics(ord,topology,coordinates)
    ...
    geometryElement = (ord == 0 ? bF(0) : (ord == 1 ? [bF(-β) bF(β)] : [bF(-γ) bF(0) bF(γ)]))
    for element = 1:size(topology,2)
...
# ╔═╡ 5ec93058-4aba-4a86-a010-074f7289cb49
begin 
#=====================================================================================
                            Mesh interpolation routine                          
=====================================================================================#
function meshInterpolation!(Fslice,Gslice,Cslice,interpolation,physicsInterpolation,
    ...
    if gOn # Only compute G if you need it
        ...
        mul!(Gslice,integrand,physicsInterpolation',true,true)  # Integration
    end
    if fOn # Only compute F if you need it
        F!(interpolation,source,k,normals,r,integrand) # Evaluate ∂ₙGreens function
        JacMul!(integrand,jacobian)                    # Multiply by jacobian*weights
        mul!(Fslice,integrand,physicsInterpolation',true,true)  # Integration
    end
...
function scaledLegendre(n,a=0.0,b=1.0)
    node, w = legendre(n)
    return (node .+ 1.0)*(b-a)/(2.0) , (b-a)*w/(2.0)
end
function createPhysicsTopology(N,physicsOrder)
    return physicsOrder < 0 ? createTopology(-physicsOrder,N) : reshape(collect(1:(physicsOrder+1)*N), physicsOrder+1, N)
end
function assemble(spline,k,n;interior=false,gOn=true,fOn=true,cOn=true,
                physicsOrder=-1,fieldPoints=[],nElements=10,N=10,β=0.8)
    # Setting up element division of Spline
    physicsFunction(ξ) = (physicsOrder == -2 ? quadratic(ξ) : 
                         (physicsOrder == -1 ? linear(ξ) : 
                         (physicsOrder ==  0 ? constant(ξ) :
                         (physicsOrder ==  1 ? discLinear(ξ,β) : discQuadratic(ξ,β)))))
    order           = max(abs(physicsOrder),1)
    tInterp         = range(1,N,length=order*nElements+1)
    nodes,weights   = scaledLegendre(n,tInterp[1],tInterp[order+1])
    physicsTopology = createPhysicsTopology(nElements,physicsOrder)
    # Compute stuff for every Gaussian node
    physicsInterpolation = physicsFunction(legendre(n)[1]')
    tGauss = kron(ones(nElements),nodes) + kron(tInterp[1:order:end-1],ones(n))
    ...
    @inbounds @views Threads.@threads for fieldNode = 1:size(fieldPoints,2)
        ...
        @inbounds @views for element = 1:nElements
            ...
            meshInterpolation!(Fslice,Gslice,Cslice,interpolation,physicsInterpolation,
                                source,k,r,normals,jacobian,integrand,gOn,fOn,cOn)
        end
    end
...
# ╔═╡ d5398af7-fd1d-485a-8bba-8d0a67eb9656
md"""
`nElements = ` $(@bind nElements html"<input type=range min=10 max=100 step=2>")

`freq 	   = ` $(@bind freq html"<input type=range min=100 max=300 step=10>")

`ElementType = ` $(@bind pO html"<select><option value=0>Constant</option><option value=-1>Linear</option><option value=-2>Quadratic</option><option value=1>Discontinuous Linear</option><option value=2>Discontinuous Quadratic</option></select>")
"""

# ╔═╡ 0d667903-d627-4043-901c-b33c96722e89
md"""
`SplineNodes = ` $(@bind Nnodes html"<input type=range min=5 max=13 step=2>")

`Stretch = ` $(@bind alpha html"<input type=range min=0.1 max=1.9 step=0.1>")

`β = ` $(@bind beta html"<input type=range min=0.3 max=1.0 step=0.05>")

`Periodic = ` $(@bind periodicTrueFalse html"<select><option value=true>True</option><option value=false>False</option></select>")
"""

# ╔═╡ 6ce15df1-bbd6-4a66-92b9-fe569e446872
begin
ntmp = Nnodes
period = parse(Bool,periodicTrueFalse)
physicsOrder = parse(Int,pO)
k = 2.0*pi*freq/(340.0); n = 6;radius = 1.0;
### Spline Geometry
coordinates = createCircle(collect(1:Nnodes-1)); coordinates = [coordinates coordinates[:,1]]
# stretchNode = Int(ceil(size(coordinates,2)/2))
# coord = coordinates[:,stretchNode]
# coordinates[:,stretchNode] = alpha*coord
spline = ParametricSpline(1:size(coordinates,2),coordinates;periodic=period)
# stretchNode = Int(ceil(size(spline.c,2)/2))
# stretchNode = [1 2 3 13 14 15]
Nspline = size(spline.c,2)
# stretchNode = [1:spline.k Nspline-spline.k+1:Nspline]
# stretchNode = [3 Nspline]
# stretchNode = [2 Nspline-1]
stretchNode = [1 Nspline-2]
tmp = spline.c[:,stretchNode]
spline.c[:,stretchNode] = alpha*tmp
F,_,C,src   = assemble(spline,k,n;physicsOrder=physicsOrder,nElements=nElements,N=ntmp,gOn=false,β=beta)

ps = (F + Diagonal(C))\exp.(im*k*src[1,:])
θ  = angle.(src[1,:] + im*src[2,:]); θ[abs.(θ) .< sqrt(eps(Float64))] .= 0.0 ;θ[θ .< 0] .+= 2π
pA = cylscat(θ,k,150)                           # Analytical sol.

plot(θ,abs.(pA), label="Analytical", title="nElements = $(nElements), ",linewidth=3,layout=2,legend=:bottom)
# scatter!(θq,abs.(pq), label="BEM-Quad",marker=:square)
plot!(θ,abs.(ps),  label="Spline",linestyle=:dash,linewidth=3)
xlims!((0,2π))
ylims!((0,3))
# scatter!(θq,abs.(pq), label="BEM-Quad",marker=:square)
plot!(θ,abs.(pA), label="Analytical",linewidth=3,subplot=2,proj=:polar,title="frequency = $(freq)")
plot!(θ,abs.(ps),  label="Spline",linestyle=:dash,linewidth=3,subplot=2,proj=:polar)
end

# ╔═╡ c6c4f08a-16e8-4489-a575-2d73b98c0c72
begin
plt=plot(eachcol(spline(1:0.1:Nnodes+1)')...,aspect_ratio=1,title="Spline Nodes = $(Nnodes), nDOFs = $(size(src,2))",label="Geometry",layout=2)
scatter!(eachcol(spline.c')...,label="Control Points")
scatter!(src[1,:],src[2,:],label="Interpolation Nodes",legend=:outerbottom)
xlims!((-2.0,2.0))
ylims!((-2.0,2.0))
plot!(plt,eachcol(spline(1:0.1:Nnodes+1)')...,aspect_ratio=1,subplot=2,legend=:outerbottom,label=false)
end

# ╔═╡ a93f3f75-fc6c-44d0-9234-d6e0fa50f6c8
spline.c

# ╔═╡ 2f443b28-85e9-4f3d-9036-8e2a7f8ceb74
discLinear(beta,beta)
...
julia_version = "1.8.3"
manifest_format = "2.0"
project_hash = "2d74d5004c70e1f59503c61bbebb58efbde652cf"
